...
Ext.define('ext.common.extjs.ux.TreeCombo', {
    ...
    records: [],
    recordData:[],// by Simon 02/07/2018
    recursiveRecords: [],
    selectChildren: true,
    canSelectFolders: true,
    multiSelect: true,
    ...
    readOnly: false,
    actionOK : function(records){},// by Simon 02/07/2018
    enableRegEx: true,
    selectItemViewHidden: false, //Helen
    searchFieldHidden: false, //Helen
    queryMode: 'local',
    emptyValue: 0, ////Helen added to fix error: Popup, click items on treeCombo, cancel, return to Popup and click items on TreeCombo, the previod checked values appear again together with this time clicked values
    /**
    ...
    //setValue: function (valueInit, valueInit1) {
        ...
        var valueFin = [];
        var values1 = []; //added by Helen, for not taking the folder code when click to folder (only take leaf codes)
        var idsFin = [];
        ...
        Ext.each(me.recursiveRecords, function (record) {
            ...
            Ext.each(value, function (val) {
                if (data == val) {
                	if (record.get('leaf') == true) //added, for: select folder, but add only its leafs to values - Helen
                    	{
                			valueFin.push(record.get(me.displayField));		//for Raw values (display values) - Helen
                			values1.push(val);			//for code values - Helen
                    	}
                    idsFin.push(data);
        ...
        //me.value = valueInit;  //removed by Helen, because this one add all values of checked node (including folders) to treeCombo value.       
        me.value = values1.join(', ');  //added by Helen, only add the value of leaf, dont add value of folders.
        
//        me.setRawValue(valueFin.join(', '));   // command by Simon 02/07/2018

        me.checkChange();
        me.emptyValue = 1; //helen: if setValue() function called, this value is set to 1.
        return me;
    },
    getValue: function () {
        return this.value;
...
/*								id: 'treeCombo_searchFieldId',
			        ...
			        },
			        {
			        	xtype: 'toolbar',
						dock: 'bottom',
						hidden : me.selectItemViewHidden,
						layout: {
							//type: 'hbox',
							//align: 'middle',
							pack: 'center'
...
//									});
									...
									// writen by : simon
									// date : 02/07/2018
									me.recordData = me.records;
									var valueFin = getListValueNodeClick(me.recordData, me.valueField, me.displayField, me.multiSelect);
									me.setRawValue(valueFin.join(', ')); 
									me.actionOK(me.recordData);
...
//									});									
    ...
    onItemClick: function (view, record, item, index, e, eOpts) {
        ...
        if (me.emptyValue == 0) me.records = []; //if first time click since the tree combo is initialized, and setValue is not called yet.
        me.emptyValue = 1; //onItemClick runs, this value is set to 1
        
        if (me.canSelectFolders == false && record.get('leaf') == false) return false;
        if (record.get('leaf') == true || me.selectChildren == false) { //if this item is leaf or the selectChildern Option is FALSE (not select Childerent)
        ...
        else {	//if if this Item is NOT leaf and selectChildrent Option is TRUE
            me.recursiveRecords = [];
            if (me.multiSelect == false || record.get('checked') == false) { //if treeCombo is single select, or 
                me.recursivePush(node);
                Ext.each(me.recursiveRecords, function (value) {
                    if (!Ext.Array.contains(me.records, value)) me.records.push(value);
                });
            }
            else if (record.get('checked') == true) {  //if treeCombo is multiple select and the Node is checked
...
// writen by : simon
// date : 02/07/2018
function getListValueNodeClick(records, valueField, displayField, multiSelect){
	var valueFin = [];
    var idsFin = [];
Ext.each(records, function (record) {
        var data = record.get(valueField);
            	if (record.get('leaf') == true) 
                	{
            			valueFin.push(record.get(displayField));		
                	}
                idsFin.push(data);
                if (multiSelect == true) record.set('checked', true);
//            }
//        });
    });
